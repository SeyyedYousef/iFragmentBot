...
function extractUsername(item) {
    try {
        ...
        let username = name.replace('.t.me', '').replace('@', '').toLowerCase().trim();
...
async function harvestAllUsernames() {
    console.log('╔═══════════════════════════════════════════════════════════════════════════════╗');
    console.log('║  ULTIMATE USERNAME HARVESTER - Starting Data Collection                      ║');
    console.log('╚═══════════════════════════════════════════════════════════════════════════════╝');
    console.log('');
    ...
    while (hasMore) {
        try {
            ...
            console.log(`   ✓ Total: ${totalFetched.toLocaleString()} | Usernames: ${allUsernames.length.toLocaleString()} | Rate: ${rate}/s`);
...
function generateStatistics(usernames) {
    ...
    stats.lengthStats.avg = (lengths.reduce((a, b) => a + b, 0) / lengths.length).toFixed(2);
...
function printStatsSummary(stats) {
    console.log('\n');
    console.log('╔═══════════════════════════════════════════════════════════════════════════════╗');
    console.log('║                         USERNAME STATISTICS SUMMARY                           ║');
    console.log('╚═══════════════════════════════════════════════════════════════════════════════╝');
    console.log('');
    ...
    for (const len of Object.keys(stats.byLength).sort((a, b) => a - b)) {
        ...
        console.log(`   ${len.padStart(2)} chars: ${d.count.toLocaleString().padStart(8)} ${bar}`);
    }
    console.log('');

    console.log('🎨 COMPOSITION:');
    console.log(`   All Letters (Clean):    ${stats.composition.allLetters.toLocaleString()}`);
    console.log(`   Has Numbers:            ${stats.composition.hasNumbers.toLocaleString()}`);
    console.log(`   Has Underscore:         ${stats.composition.hasUnderscore.toLocaleString()}`);
    console.log(`   Ends with Number:       ${stats.composition.endsWithNumber.toLocaleString()}`);
    console.log('');

    console.log('⭐ QUALITY BREAKDOWN:');
    console.log(`   ✅ Clean (valuable):    ${stats.quality.clean.toLocaleString()} (${stats.percentages.clean})`);
    console.log(`   🔷 Mixed (decent):      ${stats.quality.mixed.toLocaleString()} (${stats.percentages.mixed})`);
    console.log(`   ⚠️ Underscore (poor):   ${stats.quality.underscore.toLocaleString()} (${stats.percentages.underscore})`);
    console.log(`   🗑️ Gibberish (junk):    ${stats.quality.gibberish.toLocaleString()} (${stats.percentages.gibberish})`);
    console.log(`   ⌨️ Keyboard (trash):    ${stats.quality.keyboard.toLocaleString()} (${stats.percentages.keyboard})`);
    console.log('');
