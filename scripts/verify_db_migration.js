...
async function verify() {
	...
	if (fs.existsSync(sessionsPath)) {
		console.log(
			"⚠️ sessions.json still exists (Migration might have skipped if DB was not empty or file missing)",
		);
	...
	if (allAccounts.length > 0) {
		...
		console.log(
			`   - First Account: ${allAccounts[0].phone} (Active: ${allAccounts[0].isActive})`,
		);
