...
export function registerPanelHandlers(bot, isAdmin) {
	...
	bot.action("admin_market_stats", async (ctx) => {
		if (!isAdmin(ctx.from.id))
			return ctx.answerCbQuery("❌ Access denied").catch(() => {});
		...
		try {
			...
			const timeoutPromise = new Promise((_, reject) =>
				setTimeout(
					() => reject(new Error("Data fetch timed out (90s)")),
					90000,
				),
			);
	...
	bot.action("panel_accounts", async (ctx) => {
		...
		try {
			...
			if (!stats) {
				console.warn(
					"⚠️ Account status stats returned null/undefined, using defaults.",
				);
			}
				...
				reply_markup: {
					inline_keyboard: [
						[
							Markup.button.callback(
								"✅ بله، حذف کن",
								"adder_confirm_clear_list",
							),
							Markup.button.callback("❌ خیر", "adder_extraction_list"),
						],
					],
				},
			},
	...
	bot.action("adder_confirm_clear_list", async (ctx) => {
		...
		await ctx.editMessageText("✅ لیست با موفقیت پاک شد.", {
			reply_markup: {
				inline_keyboard: [[Markup.button.callback("🔙 بازگشت", "panel_adder")]],
			},
		});
	});
	...
	bot.action("adder_cleanup_users", async (ctx) => {
		...
		await ctx.editMessageText(
			"✅ یوزرهای تکراری و نامتبر از دیتابیس پاک شدند.",
			{
				reply_markup: {
					inline_keyboard: [
						[Markup.button.callback("🔙 بازگشت", "panel_adder")],
...
یا لیست یوزرنیم‌ها را به صورت متنی بفرستید.
        `.trim(),
			{
				parse_mode: "Markdown",
				reply_markup: {
					inline_keyboard: [[Markup.button.callback("❌ لغو", "panel_adder")]],
				},
			},
		);
	});
	...
	bot.action(/^panel_list_accounts(_page_\d+)?$/, async (ctx) => {
		...
		try {
			...
			if (ctx.callbackQuery.data.includes("_page_")) {
				page = parseInt(ctx.callbackQuery.data.split("_page_")[1], 10);
			}
			...
			if (accounts.length === 0) {
				...
				pageItems.forEach((acc, i) => {
					...
					const usernameDisplay = acc.username
						? `\`@${acc.username}\``
						: "No Username";
			...
			const keyboard = [
				...
				[
					Markup.button.callback(
						"🔄 بروزرسانی",
						`panel_list_accounts_page_${page}`,
					),
				],
	...
	bot.action("panel_account_status", async (ctx) => {
		...
		try {
			...
			if (resting.length > 0) {
				...
				if (resting.length > 5)
					msg += `_و ${resting.length - 5} مورد دیگر..._\n`;
				msg += "\n";
			}
			...
			if (reported.length > 0) {
				...
				if (reported.length > 5)
					msg += `_و ${reported.length - 5} مورد دیگر..._\n`;
			}
			...
			await ctx.editMessageText(msg, {
				...
				reply_markup: {
					inline_keyboard: [
						[
							{
								text: "♻️ پاکسازی استراحت همه",
								callback_data: "clear_all_rest",
							},
						],
	...
	bot.action("admin_add_frg", async (ctx) => {
		...
		try {
			...
			await ctx
				.answerCbQuery()
				.catch((e) => console.error("AnswerCB error:", e));
...
`)
			...
			await ctx
				.reply(`❌ Error in FRG handler: ${error.message}`)
				.catch(() => {});
		}
	});
...
`);
	...
	bot.action("admin_api_keys", async (ctx) => {
		...
		msg += `📡 *See.tg API*\n`;
		const _hasSeeTg = !!seetgAPI.updateToken; // Just a placeholder check or I can import and check cachedToken
	...
	bot.action("admin_remove_ga_token", async (ctx) => {
		...
		const tokens = giftAssetAPI.getTokenList();
		if (tokens.length === 0) {
			return ctx.editMessageText(`⚠️ No tokens to remove.`, {
				reply_markup: {
					inline_keyboard: [
						[Markup.button.callback("🔙 Back", "admin_api_keys")],
					],
				},
			});
		}
	...
	bot.action(/^admin_del_apify_(\d+)$/, async (ctx) => {
		...
		await ctx.answerCbQuery(
			removed ? "✅ Token removed" : "❌ Invalid token index",
		);
		return ctx.telegram.sendMessage(
			ctx.chat.id,
			"🔄 Updated Apify token list.",
			{
				reply_markup: {
					inline_keyboard: [
						[Markup.button.callback("🔑 API Keys", "admin_api_keys")],
