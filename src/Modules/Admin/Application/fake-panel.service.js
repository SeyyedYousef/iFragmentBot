...
import { accountStatus, orders } from "../../../database/panelDatabase.js";
 ...
 */
export async function addForcedMembersToChannel(
	targetChannel,
	userIds,
	progressCallback = null,
) {
 ...
 */
export async function massJoinChannel(
	targetChat,
	count,
	progressCallback = null,
) {
...
export async function addViews(
	...
	try {
		const accounts = accountManager
			.getAccountList()
			.filter((a) => a.status === "active");
		const _viewsPerAccount = Math.ceil(count / accounts.length);
...
export const predefinedComments = {
	...
	neutral: ["جالب بود", "ممنون", "خوب بود", "آفرین", "لایک", "👍", "❤️"],
};
