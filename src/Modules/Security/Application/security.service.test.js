...
describe('Error Sanitization', () => {
    it('should translate common Telegram errors', () => {
        assert.strictEqual(sanitizeError({ message: 'PHONE_NUMBER_INVALID' }), 'شماره تلفن نامعتبر است');
        assert.strictEqual(sanitizeError({ message: 'SESSION_EXPIRED' }), 'نشست منقضی شده. لطفاً دوباره وارد شوید');
    });

    it('should redact sensitive information', () => {
        const error = { message: 'Error with session: ABC123XYZ456 and token: bot:12345' };
