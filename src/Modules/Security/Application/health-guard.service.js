...
export function calculateTrustScore(account) {
    ...
    if (account.addedAt) {
        ...
        const daysOld = Math.floor((Date.now() - addedDate.getTime()) / (1000 * 60 * 60 * 24));
...
export function getHealthReport(accountOrPhone) {
    ...
    return {
        ...
        status,
        age: account.addedAt ? Math.floor((Date.now() - new Date(account.addedAt).getTime()) / (1000 * 60 * 60 * 24)) : 0
    };
}
...
export function recordAction(accountOrPhone, action = 'message') {
    const phone = typeof accountOrPhone === 'string' ? accountOrPhone : accountOrPhone?.phone;
    if (!phone) return;
