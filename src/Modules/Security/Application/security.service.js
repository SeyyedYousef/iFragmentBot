...
function getEncryptionKey() {
	...
	try {
		...
		console.warn(
			"⚠️ New encryption key generated (old encrypted sessions are now invalid)",
		);
	} catch (e) {
...
export async function decryptWithPassword(encryptedString, password) {
	if (!encryptedString || !password)
		throw new Error("Data and password required");
	...
	const [_version, saltHex, ivHex, authTagHex, encrypted] = parts;
	...
	check(userId, type = "default") {
		...
		if (record.count >= limit.requests) {
			const waitTime = Math.ceil(
				(limit.windowMs - (now - record.windowStart)) / 1000,
			);
...
export function withTimeout(
	promise,
	ms,
	errorMessage = "عملیات به پایان رسید",
) {
	return Promise.race([
		promise,
...
export async function withRetry(fn, maxRetries = 3, baseDelay = 1000) {
	...
	for (let attempt = 0; attempt < maxRetries; attempt++) {
		try {
			...
			if (
				noRetryPatterns.some((p) => error.message?.toUpperCase().includes(p))
			) {
				throw error;
			}
