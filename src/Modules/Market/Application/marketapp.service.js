...
export function parseGiftLink(link) {
    ...
    const tmePattern = /(?:t\.me\/nft\/|fragment\.com\/gift\/)([A-Za-z0-9_]+)-(\d+)/i;
...
async function getGiftCollections() {
    try {
        ...
        console.warn(`⚠️ API Error (getGiftCollections): ${error.message}. Using FALLBACK data.`);
        ...
        return [
            { name: 'Red Star', slug: 'red-star', extra_data: { floor: 250 }, image: 'https://cache.marketapp.ws/collection/redstar.jpg' },
            { name: 'Blue Star', slug: 'blue-star', extra_data: { floor: 85 }, image: 'https://cache.marketapp.ws/collection/bluestar.jpg' },

            // Updated Values from User Image
            { name: 'Plush Pepe', slug: 'plush-pepe', extra_data: { floor: 9200 }, image: 'https://cache.marketapp.ws/collection/plushpepe.jpg' },
            { name: 'Heart Locket', slug: 'heart-locket', extra_data: { floor: 2100 }, image: 'https://cache.marketapp.ws/collection/heartlocket.jpg' },
            { name: 'Durov\'s Cap', slug: 'durovs-cap', extra_data: { floor: 790 }, image: 'https://cache.marketapp.ws/collection/durovcap.jpg' },
            { name: 'Precious Peach', slug: 'precious-peach', extra_data: { floor: 418 }, image: 'https://cache.marketapp.ws/collection/preciouspeach.jpg' },
            { name: 'Heroic Helmet', slug: 'heroic-helmet', extra_data: { floor: 265 }, image: 'https://cache.marketapp.ws/collection/heroichelmet.jpg' },
            { name: 'Mighty Arm', slug: 'mighty-arm', extra_data: { floor: 180 }, image: 'https://cache.marketapp.ws/collection/mightyarm.jpg' },

            { name: 'Magic Potion', slug: 'magic-potion', extra_data: { floor: 45 }, image: 'https://cache.marketapp.ws/collection/magicpotion.jpg' },
            { name: 'Spy', slug: 'spy', extra_data: { floor: 18 }, image: 'https://cache.marketapp.ws/collection/spy.jpg' },
            { name: 'T-Rex', slug: 't-rex', extra_data: { floor: 65 }, image: 'https://cache.marketapp.ws/collection/trex.jpg' },
            { name: 'Lover\'s Bouquet', slug: 'lovers-bouquet', extra_data: { floor: 15 }, image: 'https://cache.marketapp.ws/collection/loversbouquet.jpg' }
        ];
    }
}
...
function getNumberBonus(itemNumber) {
    ...
    if (itemNumber % 100 === 0) return 1.10; // +10%
...
function generateAppraiserNote(estimation, attributes, extras) {
    ...
    if (premiumFactor >= 10.0) { verdict = "HOLY GRAIL"; verdictIcon = "🏆"; }
    else if (premiumFactor >= 5.0) { verdict = "MUSEUM GRADE"; verdictIcon = "🏛️"; }
    else if (premiumFactor >= 3.0) { verdict = "LEGENDARY"; verdictIcon = "🦄"; }
    else if (premiumFactor >= 2.0) { verdict = "BLUE CHIP"; verdictIcon = "💎"; }
    else if (premiumFactor >= 1.5) { verdict = "PREMIUM"; verdictIcon = "✨"; }
    else if (premiumFactor >= 1.2) { verdict = "RARE FIND"; verdictIcon = "🔍"; }
    else if (premiumFactor >= 1.05) { verdict = "ABOVE FLOOR"; verdictIcon = "📊"; }
    else if (premiumFactor < 0.9) { verdict = "BELOW MARKET"; verdictIcon = "⚠️"; }
    ...
    if (itemNumber === 1) {
        analysisParts.push(`The #1 serial confers absolute grail status — the genesis of this collection`);
    } else if (itemNumber <= 9) {
        analysisParts.push(`Single-digit serial #${itemNumber} places this among the top 0.01% of the collection`);
    } else if (itemNumber <= 100) {
        analysisParts.push(`Low serial #${itemNumber} generates significant collector demand`);
    } else if (rarityScore > 90) {
        analysisParts.push(`Exceptional rarity score (${rarityScore}/100) is the primary value driver`);
    } else if (marketData && valueVsFloor > 50) {
        analysisParts.push(`Strong market validation confirms a ${Math.round(valueVsFloor)}% premium over floor`);
    } else if (marketData && valueVsFloor > 20) {
        analysisParts.push(`Market data supports a healthy ${Math.round(valueVsFloor)}% premium`);
    } else if (modelName) {
        analysisParts.push(`The ${modelName} variant maintains a stable market position`);
    } else {
    ...
    if (marketData && marketData.totalSamples > 10) {
        analysisParts.push(`backed by ${marketData.totalSamples} comparable market data points`);
    }

    // 4. Conclusion
    if (premiumFactor >= 3.0) {
        analysisParts.push(`— a definitive collector's piece with museum-grade provenance.`);
...
function estimateGiftValue(collectionFloor, attributeFloors, attributeRarities, totalItems, onSale, extras = {}) {
    ...
    let baseValue = Math.max(collectionFloor || 0, modelFloor, backdropFloor, symbolFloor);
    let primaryDriver = 'Collection';
    if (baseValue === backdropFloor && backdropFloor > 0) primaryDriver = 'Backdrop';
    else if (baseValue === modelFloor && modelFloor > 0) primaryDriver = 'Model';
    else if (baseValue === symbolFloor && symbolFloor > 0) primaryDriver = 'Symbol';

    // Minor additive for secondary attributes (cap at 5% each, no double-count)
    if (primaryDriver !== 'Model' && modelFloor > 0 && modelFloor > collectionFloor) {
        const secondaryBonus = Math.min(modelFloor * 0.05, baseValue * 0.05);
        baseValue += secondaryBonus;
    }
    if (primaryDriver !== 'Symbol' && symbolFloor > 0 && symbolFloor > collectionFloor) {
    ...
    const MAX_PREMIUM_CAP = itemNumber <= 9 ? 2.0 : (itemNumber <= 100 ? 0.80 : 0.50);
        ...
        try {
            ...
            if (historicalSales.length >= 3) {
                ...
                if (prediction.prediction && prediction.confidence >= 50) {
                    ...
                    if (prediction.prediction > baseValue * 0.8 && prediction.prediction < baseValue * 1.5) {
            ...
            if (advancedData?.forecast?.trend === 'rising' || advancedData?.forecast?.trend === 'slightly_rising') {
                const forecastBoost = advancedData.forecast.changePercent > 10 ? 0.05 : 0.03;
    ...
    if (collectionSlug) {
        ...
        if (trendFactor !== 1.0) {
            ...
            const cappedTrend = Math.max(0.90, Math.min(1.10, trendFactor));
    ...
    if (avgRarityScore > 70) {
        let rarityPremium = 0;
        if (avgRarityScore >= 99) rarityPremium = 0.20; // +20% (was 100%)
        else if (avgRarityScore >= 95) rarityPremium = 0.12; // +12% (was 50%)
        else if (avgRarityScore >= 90) rarityPremium = 0.08; // +8% (was 20%)
    ...
    const highestFloor = Math.max(collectionFloor || 0, modelFloor, backdropFloor, symbolFloor);
    if (highestFloor > 0 && itemNumber > 9) {
        const maxSane = highestFloor * 3;
        if (estimated > maxSane) {
            console.log(`⚠️ Sanity cap applied: ${Math.round(estimated)} → ${Math.round(maxSane)} (3x highest floor ${highestFloor})`);
            estimated = maxSane;
    ...
    return {
        ...
        advancedData,
        valueRange: (() => {
            const confFactor = confidenceScore >= 80 ? 0.15 : confidenceScore >= 60 ? 0.25 : 0.35;
            return {
...
async function generateGiftReport(giftLink, tonPrice = 5.5) {
    ...
    if (!collection) {
        ...
        try {
            ...
            if (accounts.length > 0) {
                ...
                console.log(`📡 Using distributed account ${randomAccount.phone} for analysis...`);
            ...
            const { getGiftInfo } = await import('../../../Shared/Infra/Telegram/telegram.client.js');
    ...
    try {
        ...
        const [seetgGift, floorChanges, history, collectionInfo] = await Promise.all([
...
// Get market prices for similar gifts
    ...
    if (marketPrices?.backdrop?.median) medians.push(marketPrices.backdrop.median);
    if (marketPrices?.symbol?.median) medians.push(marketPrices.symbol.median);
    const medianAvg = medians.length ? (medians.reduce((a, b) => a + b, 0) / medians.length) : null;
    const gapPct = (medianAvg && medianAvg > 0) ? ((estimation.estimated / medianAvg) - 1) * 100 : null;
    if (Number.isFinite(gapPct)) {
        const label = gapPct <= -10 ? '**UNDERPRICED**' : gapPct >= 10 ? '**OVERPRICED**' : 'Market‑Aligned';
    ...
    const trendStr = (Number.isFinite(ci24) || Number.isFinite(ci7d))
        ? ` (24h: ${formatPct(ci24)} | 7d: ${formatPct(ci7d)})  (See.tg)`
        : (typeof seetgData.floorChange24h === 'number' || typeof seetgData.floorChange7d === 'number')
    ...
    if (giftAssetCap && collection.name && giftAssetCap[collection.name]?.market_cap) {
    ...
    if (seetgData.ownerUsername) {
        ...
        report += `\n`;
        if (seetgData.ownerInfo && (seetgData.ownerInfo.giftsCount || seetgData.ownerInfo.totalValue)) {
            const gc = seetgData.ownerInfo.giftsCount ? formatNumber(seetgData.ownerInfo.giftsCount) : '—';
            const tv = seetgData.ownerInfo.totalValue ? formatNumber(Math.round(seetgData.ownerInfo.totalValue)) : '—';
    ...
    if (giftAssetData && giftAssetData.providers) {
        ...
        if (providerNames.length > 0) {
            ...
            for (const pName of providerNames) {
                ...
                if (p?.collection_floor) floors.push({ pName, floor: safeNum(p.collection_floor, null) });
            }
            const validFloors = floors.filter(f => Number.isFinite(f.floor) && f.floor > 0);
            validFloors.sort((a, b) => a.floor - b.floor);
            const lowest = validFloors[0] || null;
            const highest = validFloors[validFloors.length - 1] || null;
            const spreadPct = (lowest && highest && lowest.floor > 0)
            ...
            for (const pName of providerNames) {
                ...
                const feeText = Number.isFinite(fee) ? ` (Fee ${Math.round(fee * 100)}%)` : '';
                let tag = '';
                if (lowest && pName === lowest.pName) tag = ' (LOWEST)';
                if (highest && pName === highest.pName) tag = ' (HIGHEST)';
                let line = `▸ ${pEmoji} *${pLabel}:*`;
                if (p.collection_floor) line += ` *${formatNumber(Math.round(p.collection_floor))} TON*${tag}${feeText}`;
                report += `${line}\n`;
            }
    ...
    if (collectionOffers && collectionOffers.offers && collectionOffers.offers.length > 0) {
        ...
        topOffers.forEach(offer => {
            ...
            if (net && net > 0) line += ` → Net *${formatNumber(Math.round(net))} TON*`;
            report += `${line}\n`;
        });
        report += `\n`;
    }
    ...
    if (seetgData.transfers && seetgData.transfers.length > 0) {
        ...
        for (let i = 0; i < maxOwners; i++) {
            ...
            const dateStr = transfer.date
                ...
                : 'Unknown date';
            const priceStr = transfer.price ? ` • ${formatNumber(Math.round(transfer.price))} TON` : '';
            const holdingDays = transfer.holding_days ? ` (${transfer.holding_days}d)` : '';
    ...
    if (estimation.advancedData?.forecast?.trend) {
        ...
        const label = fc.trend === 'rising' || fc.trend === 'slightly_rising' ? 'Rising' :
            (fc.trend === 'falling' || fc.trend === 'slightly_falling') ? 'Falling' : 'Stable';
    ...
    if (Array.isArray(seetgData.transfers) && seetgData.transfers.length > 0) {
        ...
        if (days.length >= 2) {
            ...
            if (dates.length >= 2) {
                ...
                if (deltas.length) avgHoldingDays = deltas.reduce((a, b) => a + b, 0) / deltas.length;
            }
        }
    }
    if (avgHoldingDays) {
    ...
    try {
        const recent = salesHistory.getSalesHistory(parsed.collectionSlug, 30) || [];
        const prices = recent
    ...
    if (marketPrices) {
        ...
        similarGifts.cheaper = uniqueSamples.filter(s => s.price < estValue).slice(-3).reverse();
        similarGifts.pricier = uniqueSamples.filter(s => s.price >= estValue).slice(0, 3);
    ...
    if (valueVsFloor < -20 && confScore >= 60) {
        ...
        signalDetail = 'Exceptional premium justified by extreme rarity and demand.';
