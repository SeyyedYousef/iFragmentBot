...
const USERNAME_COLLECTION_NAMES = ["telegram usernames", "telegram username"];
...
const GIFT_COLLECTION_ADDRESSES = new Set(
	Object.values(TELEGRAM_GIFT_COLLECTIONS),
);
...
function getValidTonApiKey() {
	...
	if (!key || key === "YOUR_TONAPI_KEY_HERE" || key.startsWith("YOUR_"))
		return null;
	return key;
}
...
async function fetchNFTsFromTonAPI(walletAddress, maxNFTs = 10000) {
	console.log(
		`⚡ TonAPI: Fetching NFTs for ${walletAddress.substring(0, 10)}...`,
	);
	...
	try {
		while (offset < maxNFTs) {
			...
			console.log(
				`⚡ TonAPI: Fetched ${nftItems.length} NFTs (offset: ${offset}, total: ${allNFTs.length})`,
			);
		...
		console.log(
			`⚡ TonAPI: Total ${allNFTs.length} NFTs fetched in ${elapsed}ms`,
		);
...
function isAnonymousNumber(nft) {
	...
	if (nft.collection?.address) {
		...
		if (
			collectionAddress.includes(
				"0a4b987e2724a41de7c25668dcafb0c4ec2ac47d0c02f202027f4715a8860752",
			)
		) {
			return true;
		}
...
export async function getOwnerWalletByUsername(username) {
	...
	try {
		...
		if (!response.ok) {
			...
			console.log(
				`⚠️ DNS resolve failed (${response.status}), trying NFT lookup...`,
			);
		...
		if (data.wallet?.address) {
			console.log(
				`✅ Found owner via DNS: ${data.wallet.address.substring(0, 10)}...`,
			);
		...
		if (data.sites && data.sites.length > 0) {
			for (const site of data.sites) {
				if (site.address) {
					console.log(
						`✅ Found owner via sites: ${site.address.substring(0, 10)}...`,
					);
					return site.address;
				}
			}
	...
	try {
		...
		if (response.ok) {
			...
			if (data.item?.owner?.address) {
				...
				console.log(
					`✅ Found owner via DNS item: ${ownerAddress.substring(0, 15)}...`,
				);
				return ownerAddress;
			}
			...
			if (data.owner?.address) {
				console.log(
					`✅ Found owner in DNS data: ${data.owner.address.substring(0, 15)}...`,
				);
			...
			if (nftAddress) {
				...
				if (nftResponse.ok) {
					...
					if (nftData.owner?.address) {
						console.log(
							`✅ Found owner via NFT lookup: ${nftData.owner.address.substring(0, 15)}...`,
						);
...
export async function getPortfolio(walletAddress) {
	console.log(
		`📂 Fetching portfolio for wallet: ${walletAddress?.substring(0, 10)}...`,
	);
	...
	try {
		...
		try {
			...
			if (accountResp.ok) {
				...
				console.warn(
					`⚠️ TonAPI account fetch failed (${accountResp.status}), trying Toncenter...`,
				);
				const tcResp = await fetchWithRetry(
					`${TONCENTER_BASE_URL}/account?address=${encodeURIComponent(walletAddress)}`,
				);
				if (tcResp.ok) {
					const tcData = await tcResp.json();
					accountData = {
						balance: parseInt(tcData.balance || "0", 10),
						status: tcData.status || "active",
						is_wallet: true,
						last_activity: tcData.last_transaction_lt
		...
		for (const nft of nfts) {
			...
			if (isAnonymousNumber(nft)) {
				...
				if (
					number &&
					!portfolio.anonymousNumbers.find((n) => n.number === number)
				) {
			...
			const displayName = OFFICIAL_GIFT_NAMES.has(cleanName)
				? cleanName
				: collectionName || "Unknown Gift";
		...
		portfolio.totalGifts = portfolio.gifts.reduce(
			(sum, g) => sum + (g.count || 1),
			0,
		);
		...
		const totalValue =
			portfolio.totalUsernames * EST_MIN_USERNAME_VALUE +
		...
		console.log(
			`✅ Portfolio: ${portfolio.totalUsernames} usernames, ${portfolio.totalNumbers} numbers, ${portfolio.totalGifts} gifts (Value: ~${totalValue} TON) in ${elapsed}ms`,
		);
