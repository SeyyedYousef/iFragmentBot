...
import { CONFIG, getSuggestions, GOLDEN_DICTIONARY } from '../../../core/Config/app.config.js';
 ...
 */
/**
 * Scrape Fragment with Hybrid Approach (HTTP First -> Puppeteer Fallback)
...
export async function scrapeFragment(username) {
    ...
    return fragmentLimiter.schedule(async () => {
        ...
        try {
            ...
            if (httpData && (httpData.status !== 'unknown' || httpData.priceTon > 0)) {
                console.log(`⚡ HTTP Scrape SUCCESS for @${cleanUsername}`);
                return httpData;
            }
            console.log(`⚠️ HTTP Scrape inconclusive for @${cleanUsername}, falling back to Puppeteer...`);
        } catch (httpError) {
            console.warn(`⚠️ HTTP Scrape failed: ${httpError.message}, falling back to Puppeteer...`);
        }

        // 2. SLOW PATH: Puppeteer (Fallback)
                ...
                if (!result.ownerWallet) {
                    ...
                    for (const link of tonviewerLinks) {
                        ...
                        if (match) {
                            ...
                            if (row && row.innerText.includes('Bid')) continue;
                ...
                if (!result.lastSalePrice) {
                    ...
                    for (const row of rows) {
                        ...
                        if (text.includes('sold') || text.includes('purchased')) {
                            ...
                            if (priceNode) {
                                const priceText = priceNode.parentElement.innerText.replace(/[^\d.]/g, '');
                                const price = parseFloat(priceText);
                                if (price > 0) {
                                    result.lastSalePrice = price;
                                    ...
                                    if (dateNode) result.lastSaleDate = dateNode.getAttribute('datetime');
...
async function scrapeFragmentHttp(username) {
    ...
    if (response.status === 404) {
        // Determine if it's truly unregistered or just not found
        // For now, treat as available/unregistered if 404
    ...
    // 1. Status Detection
    const statusMatch = html.match(/class="[^"]*tm-section-header-status[^"]*"[^>]*>([^<]+)<\/span>/i);
    if (statusMatch) {
    ...
    // 2. Price Extraction
    // Auction: Minimum bid or Highest bid
    // Sale: Sale price

    // Look for "icon-ton">123</div> pattern
    // We capture ALL prices and try to infer context
    const priceMatches = [...html.matchAll(/icon-ton">([\d,]+(?:\.\d+)?)<\/div>/g)];

    // Attempt to extract specific prices based on context search

    // Highest Bid (in Bid Info table)
    if (result.status === 'on_auction') {
        // Often the first price in the table key-values is highest bid, then bid step, then min bid
        ...
        // Sold price is often in history or header
        // Check regex for "Sold for..."
        const soldMatch = html.match(/Sold for\s+<span[^>]*class="icon-ton"[^>]*>([\d,]+)/i);
        if (soldMatch) {
            result.lastSalePrice = parseFloat(soldMatch[1].replace(/,/g, ''));
        } else if (priceMatches.length > 0) {
            // First price might be it
    ...
    if (!result.ownerWalletFull) {
        ...
        if (walletMatch) {
            ...
            result.ownerWallet = result.ownerWalletFull.substring(0, 6) + '...' + result.ownerWalletFull.slice(-4);
        }
    }
    ...
    if (timeMatch) {
        if (result.status === 'on_auction') {
            result.auctionEnds = timeMatch[1]; // Ends at
        } else if (result.status === 'sold') {
            result.lastSaleDate = timeMatch[1].split('T')[0]; // Sold at
        }
    }

    // 5. JSON State Falback (The "Super" Part)
    // Extract shared state from <script>ajInit({...})</script>
    ...
    if (jsonMatch) {
        try {
            ...
            if (json.state) {
                ...
                if (s.lt) result.auctionLastLt = s.lt; // Internal ID
                // Sometimes useful hidden data is here
            }
 ...
 */
/**
 * Scrape Gift Owner from Fragment/Telegram (HTTP Version - High Performance)
 */
export async function scrapeGiftOwner(slug, itemNumber) {
    return fragmentLimiter.schedule(async () => {
        ...
        try {
            ...
            const ownedMatch = html.match(/Owned by\s+(?:<a[^>]*>)?@?([a-zA-Z0-9_]+)(?:<\/a>)?/i);
            if (ownedMatch) {
            ...
            if (!result.username) {
                ...
                const links = [...html.matchAll(/href="https:\/\/t\.me\/([a-zA-Z0-9_]+)"/g)];
                for (const match of links) {
                    const candidate = match[1];
                    // Filter out reserved words or likely non-user paths if necessary
                    if (candidate.toLowerCase() !== 'nft' && candidate.toLowerCase() !== 'iv') {
            ...
            if (result.username) {
                ...
                console.log(`❌ HTTP Scraper could not find owner for ${slug}-${itemNumber}`);
                return null;
            }
...
export async function getTonMarketStats() {
    ...
    try {
        const response = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=the-open-network&vs_currencies=usd&include_24hr_change=true', { timeout: 5000 });
        if (response.ok) {
