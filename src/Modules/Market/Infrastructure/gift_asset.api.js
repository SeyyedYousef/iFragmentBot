...
class GiftAssetAPI {
	...
	async _loadTokens() {
		try {
			...
			if (dbTokens && dbTokens.length > 0) {
				...
				console.log(
					`🔑 [GiftAsset] Loaded ${this.tokens.length} API tokens from MongoDB`,
				);
				return;
			}
	...
	async addToken(token) {
		if (token && token.length > 5 && !this.tokens.includes(token)) {
			...
			if (success) {
				...
				console.log(
					`✅ [GiftAsset] Token added to DB: ${token.substring(0, 8)}...`,
				);
				return true;
			}
		}
	...
	async removeToken(index) {
		if (index >= 0 && index < this.tokens.length) {
			...
			if (success) {
				...
				console.log(
					`🗑️ [GiftAsset] Token removed from DB: ${tokenToDelete.substring(0, 8)}...`,
				);
				return true;
			}
		}
	...
	async _getNextToken() {
		...
		if (bestTokenIdx !== -1 && shortestWait > 0 && shortestWait < 20000) {
			// Max queue hold 20s
			console.log(
				`⏳ [GiftAsset Engine] 🚦 Traffic Jam! System queuing for ${Math.ceil(shortestWait / 1000)}s until Token #${bestTokenIdx} resets...`,
			);
			...
			console.log(
				`🚨 [GiftAsset Engine] Heavy Overload! Forcing emergency override on Token #${this.currentTokenIndex}...`,
			);
	...
	async _request(method, endpoint, params = {}, body = null) {
		...
		for (let attempt = 0; attempt < maxAttempts; attempt++) {
			...
			try {
				...
				if (!Number.isNaN(remaining) && remaining <= 1 && tokenInfo) {
					this.cooldowns.set(tokenInfo.index, Date.now() + 15000); // 15s proactive rest
					console.log(
						`🛡️ [GiftAsset Engine] Token #${tokenInfo.index} critically low on rates. Entering proactive sleep.`,
					);
				}
				...
				if (response.status === 429 || response.status === 401) {
					...
					if (retryAfter) {
						penaltyTime = parseInt(retryAfter, 10) * 1000;
					...
					if (tokenInfo) {
						...
						console.log(
							`🔥 [GiftAsset Engine] Token #${tokenInfo.index} hit 429/401! Banning for ${penaltyTime / 1000}s. Spinning cylinder...`,
						);
					}
				...
				if (!response.ok) {
					...
					console.warn(
						`[GiftAsset Engine] Error ${response.status} on ${endpoint}: ${errorText.substring(0, 60)}`,
					);

					if (response.status >= 500 && attempt < maxAttempts - 1) {
						// Server transient fail, wait a moment and hit again
						const backoff = 1000 * 1.5 ** attempt;
				...
				console.warn(
					`[GiftAsset Engine] TCP/Network error on ${endpoint} ->`,
					error.message,
				);
		...
		console.error(
			"❌ [GiftAsset Engine] FATAL: All tokens burned out and limits exceeded.",
		);
		return null;
	}
	...
	async getGiftsByUser(username, limit = 100, offset = 0) {
		return this._request("GET", "/get_gift_by_user", {
			username,
			limit,
			offset,
		});
	...
	async getUserProfilePrice(username, limit = 100, offset = 0) {
		return this._request("GET", "/get_user_profile_price", {
			username,
			limit,
			offset,
		});
	...
	async getCustomCollectionsVolumes(maxTime) {
		return this._request("GET", "/get_custom_collections_volumes", {
			maxtime: maxTime,
		});
	}
	...
	async getProvidersSalesHistory(
		providerName,
		limit = 50,
		offset = 0,
		premarket = false,
