...
class JobQueue {
    constructor() {
        ...
        this.statsInterval = setInterval(() => this.logStats(), CONFIG.STATS_LOG_INTERVAL);

        console.log('📋 Job Queue initialized');
        console.log(`   Heavy jobs (Puppeteer): max ${CONFIG.MAX_PUPPETEER_CONCURRENCY} concurrent`);
        console.log(`   Light jobs (API): max ${CONFIG.MAX_API_CONCURRENCY} concurrent`);
    }
    ...
    async add(options) {
        ...
        if (type === JOB_TYPES.GIFT_REPORT && data.link) {
            for (const job of this.jobs.values()) {
                if (
                    (job.status === JOB_STATUS.QUEUED || job.status === JOB_STATUS.PROCESSING) &&
                    job.type === JOB_TYPES.GIFT_REPORT &&
                    job.data && job.data.link === data.link
                ) {
        ...
        console.log(`📋 Job added: ${jobId} (${type}) for user ${userId} [priority: ${priority}]`);
     ...
     */
    async notifyJobResult(job, result, error) {
    ...
    getStats() {
        return {
            ...
            jobs: {
                ...
                queued: [...this.jobs.values()].filter(j => j.status === JOB_STATUS.QUEUED).length,
                processing: [...this.jobs.values()].filter(j => j.status === JOB_STATUS.PROCESSING).length,
            },
            stats: {
    ...
    logStats() {
        ...
        if (stats.heavyQueue.pending > 0 || stats.lightQueue.pending > 0) {
            console.log(`📊 Queue: Heavy[${stats.heavyQueue.pending}] Light[${stats.lightQueue.pending}] | ` +
...
export function formatQueueMessage(position, estimatedWait, isPremium = false) {
    ...
    const priorityNote = isPremium
        ? '⚡ _Premium Priority Active_'
        : '';
