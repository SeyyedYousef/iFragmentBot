				...
				for (const chat of results.chats) {
					if (
						...
						const exists = await db
							.collection("discovered_groups")
							.findOne({ username: chat.username });
						if (!exists) {
							const groupData = {
								id: chat.id.toString(),
								title: chat.title,
...
export async function joinGroupUniquely(groupId) {
	...
	const alreadyOccupied = await db
		.collection("group_occupancy")
		.findOne({ groupUsername: group.username });
	if (alreadyOccupied)
		return {
			success: false,
			error: "Group already occupied by another account",
		};
	...
	for (const acc of accounts) {
		const count = await db
			.collection("group_occupancy")
			.countDocuments({ accountPhone: acc.phone });
		if (count < 10) {
			// Check if this account already in THIS group (extra safety)
			const inThis = await db
				.collection("group_occupancy")
				.findOne({ accountPhone: acc.phone, groupUsername: group.username });
			if (!inThis) {
				bestAccount = acc;
				break;
			}
	...
	if (!bestAccount)
		return { success: false, error: "No account with capacity available" };
	...
	try {
		...
		await db
			...
			.updateOne(
				{ _id: groupId },
				{ $set: { status: "joined", occupiedBy: bestAccount.phone } },
			);
...
export async function broadcastToGroups(message) {
	...
	for (const membership of joined) {
		try {
			const client = await accountManager.getClientByPhone(
				membership.accountPhone,
			);
			if (client) {
			...
			console.warn(
				`Broadcast failed for ${membership.groupUsername}: ${e.message}`,
			);
			failed++;
		}
	}
