...
async function postDailyReport() {
	...
	try {
		...
		if (imageBuffer && imageBuffer.length > 0) {
			console.log(
				`📤 Daily Scheduler: Sending photo report to ${CHANNEL_ID} (${imageBuffer.length} bytes)...`,
			);
			...
			console.log(
				`📨 Daily Scheduler: Sending text-only report to ${CHANNEL_ID} (Image buffer empty or missing)`,
			);
...
export function startScheduler() {
	...
	const afgTime = getAfghanistanTime();
	console.log(
		`⏰ Daily Scheduler started (target: ${TARGET_HOUR}:00 AFG, now: ${afgTime.hours}:${String(afgTime.minutes).padStart(2, "0")})`,
	);
