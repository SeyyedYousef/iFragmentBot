...
function getNextAccount() {
	...
	for (const acc of accounts) {
		...
		if (usage.adds < limit && usage.adds < minUsage) {
			minUsage = usage.adds;
			bestAccount = acc;
		}
	}
...
// ==================== OWNER EXTRACTION (Enhanced with Native MTProto) ====================
async function extractOwnerFromGift(client, collectionSlug, itemNumber) {
	try {
		const giftId = `${collectionSlug}-${itemNumber}`;
		const _giftLink = `https://t.me/nft/${giftId}`;

		console.log(`🔍 Extracting: ${giftId} via Native Telegram...`);

		// Phase 1: Try Native MTProto Check (Fastest & Accurate)
		try {
			// Using GetCollectibleInfo - this is the native way to check NFT/Collectibles
			const result = await client.invoke(
				new Api.messages.GetCollectibleInfo({
					collectible: new Api.InputCollectibleUsername({
						username: giftId,
					}),
				}),
			);
			...
			if (result?.user) {
				...
				console.log(`✅ Native found: @${user.username || user.id}`);
				return {
					id: user.id?.toString(),
			...
			console.log(
				`ℹ️ Native check failed for ${giftId}: ${e.message}. Falling back to scraper...`,
			);
		}

		// Phase 2: Fallback to HTTP Scraper (Previous method)
		...
		if (scrapedData?.username) {
			...
			try {
				...
				console.log(
					`⚠️ User @${username} found by scraper but not resolvable: ${e.message}`,
				);
				return {
					id: null,
					username,
		...
		console.log(
			`⚠️ Extract error for ${collectionSlug}-${itemNumber}: ${error.message}`,
		);
		return null;
	}
}
...
async function extractOwnersFromCollection(
	collectionSlug,
	startNum,
	endNum,
	...
	// We don't get a single client here anymore, we rotate per item
	// Check if we have ANY accounts
	const stats = accountManager.getPoolStats();
	if (stats.active === 0) {
	...
	try {
		for (let i = startNum; i <= endNum; i++) {
			...
			try {
				...
				}, "scanner"); // Use 'scanner' role accounts
			} catch (err) {
				console.log(`⚠️ Item #${i} failed: ${err.message}`);
			}
...
async function extractMembersFromChat(chatLink, progressCallback = null) {
	...
	try {
			...
			if (chatLink.includes("+") || chatLink.includes("joinchat")) {
				...
				try {
					...
					if (e.message.includes("ALREADY")) {
						...
						const _dialogs = await client.getDialogs({ limit: 100 });
		...
		console.log(
			`📋 Extracting members from: ${chatEntity.title || chatEntity.username}`,
		);
			...
			try {
				...
				if (progressCallback) {
					progressCallback({
						...
						found: members.size,
						percent: participants.count
							? Math.round((offset / participants.count) * 100)
							: 0,
					});
				}
				...
				await new Promise((r) =>
					setTimeout(r, CONFIG.CHAT_EXTRACTION_DELAY_MS),
				);
			} catch (e) {
...
async function importFromCSV(csvContent) {
	try {
		...
		for (const line of lines) {
			...
			if (
				username.length >= 3 &&
				/^[a-zA-Z][a-zA-Z0-9_]{2,30}$/.test(username)
			) {
...
async function addAllToContacts(progressCallback = null) {
	...
	for (const contact of pending) {
		...
		if (!account) {
			return {
				success: false,
				error: "No available accounts (daily limit reached)",
			};
		}
	...
	for (const contact of pending) {
		...
		try {
			...
			if (chat) {
				...
				await db.collection("extracted_contacts").updateOne(
					{ username: contact.username },
					{
						$set: {
							addedToGroup: true,
							addedToGroupAt: new Date(),
							addedToGroupBy: account.phone,
						},
					},
				);
			...
			if (e.message?.includes("FLOOD_WAIT")) {
				const wait = parseInt(e.message.match(/\d+/)?.[0] || 60, 10) * 1000;
...
async function getExtractedList(limit = 50, skip = 0) {
	...
	const addedToContacts = await db
		.collection("extracted_contacts")
		.countDocuments({ addedToContacts: true });
	const addedToGroup = await db
		.collection("extracted_contacts")
		.countDocuments({ addedToGroup: true });
...
// ==================== EXTRACT FROM LINK (Enhanced) ====================
async function extractOwnerFromGiftLink(link) {
	try {
		const match = link.match(/t\.me\/nft\/([^/-]+)-(\d+)/i);
		if (!match) {
			console.log(`⚠️ Invalid gift link format: ${link}`);
			return null;
		}

		const slug = match[1];
		const itemNumber = parseInt(match[2], 10);
		const giftId = `${slug}-${itemNumber}`;

		console.log(`🔍 Extracting from link natively: ${giftId}`);

		const client = await accountManager.getClient("scanner");
		if (!client) {
			console.log("⚠️ No scanner account available");
			return null;
		}
		...
		try {
			...
			console.log(
				`ℹ️ Native link check failed: ${e.message}. Falling back to scraper...`,
			);
		}

		// Fallback to Scraper
		const scraped = await scrapeGiftOwner(slug, itemNumber);
		if (scraped?.username) {
			const username = scraped.username;
			...
			return { userId: entity?.id?.toString() || null, username };
		}

		return null;
	} catch (error) {
		console.error(`❌ Extract link error: ${error.message}`);
		return null;
	}
}
