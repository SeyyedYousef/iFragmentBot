...
import { orders, proxies, accountStatus } from '../../../database/panelDatabase.js';
...
async function extractOwnerFromGift(client, collectionSlug, itemNumber) {
    try {
        ...
        const giftLink = `https://t.me/nft/${giftId}`;
        ...
        try {
            ...
            const result = await client.invoke(new Api.messages.GetCollectibleInfo({
            ...
            console.log(`ℹ️ Native check failed for ${giftId}: ${e.message}. Falling back to scraper...`);
        }
        ...
        if (scrapedData && scrapedData.username) {
            ...
            try {
                ...
                console.log(`⚠️ User @${username} found by scraper but not resolvable: ${e.message}`);
                return {
                    id: null,
                    username,
                    firstName: username,
        ...
        console.log(`⚠️ Extract error for ${collectionSlug}-${itemNumber}: ${error.message}`);
        return null;
    }
}
...
async function extractOwnerFromGiftLink(link) {
    try {
        ...
        const slug = match[1];
        const itemNumber = parseInt(match[2]);
        ...
        try {
            ...
            console.log(`ℹ️ Native link check failed: ${e.message}. Falling back to scraper...`);
        }
