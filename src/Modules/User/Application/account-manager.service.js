...
const clients = new Map(); // Map<phoneNumber, TelegramClient>
...
export async function loadAccounts() {
	try {
		const {
			loadAccounts: loadAccountsFromMongo,
			countAccounts: countFromMongo,
		} = await import("../../../Shared/Infra/Database/mongo.repository.js");
		...
		accounts = dbRows.map((row) => {
			...
			try {
				if (session?.includes(":") && session.length > 50) {
...
async function saveAccountToDb(account) {
	try {
		const { saveAccount: saveAccountToMongo } = await import(
			"../../../Shared/Infra/Database/mongo.repository.js"
		);
...
export async function initAccounts() {
	...
	if (!apiId || !apiHash) {
		console.warn(
			"⚠️ [AccountManager] API_ID/HASH missing. Account features will be disabled.",
		);
		isInitialized = true;
	...
	isInitialized = true;
	console.log(
		`✅ Account Pool Ready: ${connected}/${accounts.filter((a) => a.isActive).length} connected, ${failed} failed.`,
	);
...
async function checkIdleClients() {
	...
	for (const [phone, client] of clients.entries()) {
		...
		if (client.lastUsed && now - client.lastUsed > IDLE_TIMEOUT_MS) {
			try {
				...
				console.error(
					`Error disconnecting idle client ${hashForLog(phone)}`,
					e,
				);
			}
		}
	...
	if (disconnectedCount > 0) {
		console.log(
			`🧹 Disconnected ${disconnectedCount} idle accounts to save memory.`,
		);
	...
	try {
		...
		if (account.proxy) {
			clientParams.proxy = {
				...
				port: parseInt(account.proxy.port, 10),
		...
		console.error(
			`❌ Failed to connect ${hashForLog(account.phone)}:`,
			error.message,
		);
		account.status = "error";
...
export async function addSession(phoneNumber, sessionString, user) {
	...
	try {
		...
		console.error("Error saving new account to DB", e);
	}
...
export async function getClient(role = "all") {
	...
	if (active) {
		// Update stats
		const _account = accounts.find((a) => a.phone === active.phone);
...
export function getActiveClient(role = "all") {
	...
	// Update stats
	selectedAccount.stats.requestsToday =
		(selectedAccount.stats.requestsToday || 0) + 1;
	selectedAccount.stats.totalRequests =
		(selectedAccount.stats.totalRequests || 0) + 1;
	selectedAccount.stats.lastActive = now;

	const client = clients.get(selectedAccount.phone);
...
export async function reconnectAll() {
	const disconnected = accounts.filter(
		(a) => a.isActive && a.session && !clients.has(a.phone),
	);
...
export async function getExportableAccounts(password) {
	...
	const plainAccounts = accounts.map((acc) => {
		...
		if (
			...
			cleanSession.length < 300 &&
			!isValidSessionString(cleanSession)
		) {
...
export async function importAccounts(importData, password) {
	...
	for (const accData of accountsData) {
		try {
			...
			const isEncrypted =
				accData.encrypted ||
				(sessionString.includes(":") && sessionString.length < 300);
