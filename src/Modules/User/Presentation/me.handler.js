...
export async function handleMeCommand(ctx) {
    ...
    if (isCallback && ctx.callbackQuery.data.startsWith('me_page_')) {
        page = parseInt(ctx.callbackQuery.data.split('_')[2]);
    }
    ...
    if (isCallback) {
        try {
            ...
            loadingMsg = await ctx.reply('⏳ *Consulting the Oracle...* \nScanning blockchain for assets...', { parse_mode: 'Markdown' });
        }
    } else {
    ...
    try {
        ...
        const isFreshRequest = !isCallback || ctx.callbackQuery.data === 'refresh_me';
        const loadingMsgId = loadingMsg ? loadingMsg.message_id : undefined; // Define loadingMsgId here

        if (!isFreshRequest && ctx.session.meCache[cacheKey] && (Date.now() - ctx.session.meCache[cacheKey].timestamp < 300 * 1000)) { // 5 minutes cache
            ...
            if (!userGiftsResult.success) {
                ...
                if (loadingMsgId) {
                    try { await ctx.telegram.editMessageText(ctx.chat.id, loadingMsgId, null, errorText, { parse_mode: 'Markdown' }); } catch (e) { await ctx.reply(errorText, { parse_mode: 'Markdown' }); }
                } else { await ctx.reply(errorText, { parse_mode: 'Markdown' }); }
                return;
            }
        ...
        const tonPrice = tonPriceData?.price || 5.50;
        const euroRate = 0.92;
        ...
        for (const gift of gifts) {
            ...
            if (collectionData) {
                ...
                if (gift.backdrop && collectionData.Backdrop && collectionData.Backdrop[gift.backdrop]) {
                    valueTon = collectionData.Backdrop[gift.backdrop];
                } else if (gift.model && collectionData.Model && collectionData.Model[gift.model]) {
        ...
        const tonInEur = tonPrice * euroRate;
        ...
        const sortedGroups = Object.values(groupedGifts).sort((a, b) => b.valueTon - a.valueTon);
        ...
        if (pageItems.length > 0) {
            pageItems.forEach((item, index) => {
                const link = item.slug ? `https://fragment.com/gift/${item.slug}` : `https://fragment.com`;
                ...
                if (item.name.toLowerCase().includes('pepe')) icon = '🐸';
                    ...
                    if (item.backdrop && item.backdrop.toLowerCase().includes('brown')) icon = '🟫';
                    else if (item.backdrop && item.backdrop.toLowerCase().includes('purple')) icon = '🟪';
                    else if (item.backdrop && item.backdrop.toLowerCase().includes('blue')) icon = '🟦';
                }
                ...
                const displayName = item.model ? `${item.name} (${item.model})` : item.name;
        ...
        // Paging Buttons Row
        const navRow = [];
        if (page > 1) navRow.push({ text: '⬅️ Previous', callback_data: `me_page_${page - 1}` });
        if (page < totalPages) navRow.push({ text: '➡️ Next', callback_data: `me_page_${page + 1}` });
        if (navRow.length > 0) keyboard.inline_keyboard.push(navRow);

        // Actions Row
        keyboard.inline_keyboard.push([{ text: '🔄 Refresh Assets', callback_data: 'refresh_me' }]);

        // Send/Edit Message
        const msgIdToEdit = loadingMsg ? loadingMsg.message_id : undefined;
        if (msgIdToEdit) {
            try {
                await ctx.telegram.editMessageText(ctx.chat.id, msgIdToEdit, null, report, {
                    parse_mode: 'Markdown',
        ...
        if (loadingMsg && loadingMsg.message_id) {
            try { await ctx.telegram.editMessageText(ctx.chat.id, loadingMsg.message_id, null, errorText, { parse_mode: 'Markdown' }); } catch (e) { }
        } else {
