...
function getPaginatedAccountsKeyboard(accounts, page = 0, callbackPrefix) {
	...
	if (totalPages > 1) {
		...
		if (page > 0) {
			navButtons.push({
				text: "◀️ قبلی",
				callback_data: `${callbackPrefix}_page:${page - 1}`,
			});
		}
		navButtons.push({
	...
	bot.action("panel_add_session", async (ctx) => {
		...
		if (!rateCheck.allowed) {
			return ctx.answerCbQuery(
				`⏳ لطفاً ${rateCheck.waitSeconds} ثانیه صبر کنید`,
			);
		}
        ...
        `.trim(),
			{
				...
				reply_markup: {
					inline_keyboard: [
						[
							{
								text: "💾 افزودن با Session",
								callback_data: "panel_add_session",
							},
						],
...
اکانت مورد نظر برای حذف را انتخاب کنید:
        `.trim(),
			{
				parse_mode: "Markdown",
				reply_markup: getPaginatedAccountsKeyboard(
					accounts,
					page,
					"remove_acc",
				),
				...
				reply_markup: {
					inline_keyboard: [
						[
							{
								text: "✅ بله، حذف شود",
								callback_data: `confirm_remove:${phone}`,
							},
	...
	bot.action(/^confirm_remove:(.+)$/, async (ctx) => {
		...
		try {
			...
			if (success) {
				...
				console.log(
					`Account removed: ${hashForLog(phone)} by admin ${ctx.from.id}`,
				);
			} else {
				...
				reply_markup: {
					inline_keyboard: [
						[
							{
								text: "✨ هویت هوشمند (AI)",
								callback_data: "account_profile_ai",
							},
						],
						[
							{
								text: "📝 تغییر دستی",
								callback_data: "account_profile_manual",
							},
	...
	bot.action(/^apply_ai:(.+)$/, async (ctx) => {
		...
		for (const acc of accounts) {
			try {
				...
				// Apply it
				const result = await profileManager.applyProfileToAccount(
					acc.phone,
					profileId,
				);
				...
				if (result.status === "fulfilled") {
					...
					if (health.connected && !health.error) {
						...
						results.results.push({
							phone: health.phone,
							status: "🔴",
							detail: sanitizeError(health.error),
						});
					}
				} else {
					results.issues++;
...
اکانت مورد نظر را انتخاب کنید:
	...
	bot.action(/^manage_acc:(.+)$/, async (ctx) => {
		...
		const lastActivity = status?.last_activity
			? new Date(status.last_activity).toLocaleString("fa-IR")
			: "نامشخص";
				...
				reply_markup: {
					inline_keyboard: [
						[
							{
								text: "🔑 کد ورود",
								callback_data: `panel_get_code_one:${phone}`,
							},
							{
								text: "🔍 چک سلامت",
								callback_data: `check_health_one:${phone}`,
							},
						],
						[
						...
						[
							{
								text: "🔙 لیست اکانت‌ها",
								callback_data: "panel_manage_selection",
							},
						],
					],
				},
...
async function handleBackupPassword(ctx, text, _state) {
	...
	try {
		...
		const encryptedSessions =
			await accountManager.getExportableAccounts(password);
...
async function handleRestorePassword(ctx, text, state) {
	...
	try {
		...
		const result = await accountManager.importAccounts(
			backupData.sessions,
			password,
		);
...
async function handleSessionString(ctx, text) {
	...
	for (const session of lines.slice(0, 10)) {
		// Max 10 at once
...
• فرمت Base64 صحیح داشته باشد`);
	...
	const progressMsg = await ctx.reply(
		`⏳ در حال افزودن ${validSessions.length} اکانت...`,
	);
	...
	console.log(
		`Sessions added: ${success} success, ${failed} failed by admin ${ctx.from.id}`,
	);
...
USER نام کاربری: \`${result.user.username ? `@${result.user.username}` : "ندارد"}\``,
				...
				console.log(
					`Account added via phone: ${hashForLog(state.phone)} by admin ${ctx.from.id}`,
				);
...
USER نام کاربری: \`${result.user.username ? `@${result.user.username}` : "ندارد"}\``,
			...
			console.log(
				`Account added via 2FA: ${hashForLog(state.phone)} by admin ${ctx.from.id}`,
			);
		} else {
			await ctx.telegram.editMessageText(
				ctx.chat.id,
...
export async function handleAccountFileMessage(ctx, state) {
	...
	try {
		const _file = await ctx.telegram.getFile(doc.file_id);
		...
		const isEncrypted =
			backupData.encrypted || typeof backupData.sessions === "string";
...
🔐 نسخه: \`${backupData.version || "Legacy"}\``,
		...
		console.log(
			`Backup restored: ${result.success} accounts by admin ${ctx.from.id}`,
		);
	} catch (error) {
		await ctx.telegram.editMessageText(
			ctx.chat.id,
