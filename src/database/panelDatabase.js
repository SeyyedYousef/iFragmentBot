...
const profileQueries = {
    ...
    getUnused: db.prepare('SELECT * FROM profiles WHERE is_used = 0 ORDER BY RANDOM() LIMIT 1'),
    ...
    countUnused: db.prepare('SELECT COUNT(*) as count FROM profiles WHERE is_used = 0')
};

export const profiles = {
    add(firstName, lastName = '', bio = '', photoPath = '', username = '') {
        const result = profileQueries.insert.run(firstName, lastName, bio, photoPath, username);
...
const orderQueries = {
    ...
    updateStatus: db.prepare('UPDATE orders SET status = ?, finished_at = CURRENT_TIMESTAMP WHERE id = ?'),
    setError: db.prepare('UPDATE orders SET status = ?, error_message = ?, finished_at = CURRENT_TIMESTAMP WHERE id = ?'),
...
const proxyQueries = {
    ...
    getRandom: db.prepare('SELECT * FROM proxies WHERE is_active = 1 ORDER BY RANDOM() LIMIT 1'),
    toggle: db.prepare('UPDATE proxies SET is_active = ? WHERE id = ?'),
    updateLastUsed: db.prepare('UPDATE proxies SET last_used = CURRENT_TIMESTAMP WHERE id = ?'),
    delete: db.prepare('DELETE FROM proxies WHERE id = ?'),
    deleteAll: db.prepare('DELETE FROM proxies'),
    count: db.prepare('SELECT COUNT(*) as count FROM proxies WHERE is_active = 1')
};

export const proxies = {
    add(type, host, port, username = '', password = '') {
        const result = proxyQueries.insert.run(type, host, port, username, password);
        return result.lastInsertRowid;
    },
...
const accountStatusQueries = {
    ...
    getHealthy: db.prepare('SELECT * FROM account_status WHERE is_reported = 0 AND is_resting = 0'),
    setReported: db.prepare('UPDATE account_status SET is_reported = 1, report_count = report_count + 1, updated_at = CURRENT_TIMESTAMP WHERE phone = ?'),
    setResting: db.prepare('UPDATE account_status SET is_resting = 1, rest_until = ?, updated_at = CURRENT_TIMESTAMP WHERE phone = ?'),
    clearRest: db.prepare('UPDATE account_status SET is_resting = 0, rest_until = NULL, updated_at = CURRENT_TIMESTAMP WHERE phone = ?'),
    clearAllRest: db.prepare('UPDATE account_status SET is_resting = 0, rest_until = NULL, updated_at = CURRENT_TIMESTAMP'),
...
export const accountStatus = {
    set(phone, { isReported = 0, isResting = 0, restUntil = null, proxyId = null, folder = 'default' } = {}) {
        accountStatusQueries.upsert.run(phone, isReported, isResting, restUntil, proxyId, folder);
    },
...
const receiverQueries = {
    insert: db.prepare(`
        ...
        VALUES (?, ?, ?)
    `),
    getAll: db.prepare('SELECT * FROM receiver_accounts ORDER BY donated_at DESC'),
    getPending: db.prepare('SELECT * FROM receiver_accounts WHERE is_approved = 0'),
    getApproved: db.prepare('SELECT * FROM receiver_accounts WHERE is_approved = 1'),
    approve: db.prepare('UPDATE receiver_accounts SET is_approved = 1, approved_at = CURRENT_TIMESTAMP WHERE id = ?'),
...
export const receiver = {
    add(phone, sessionString, donatedBy) {
        try {
            const result = receiverQueries.insert.run(phone, sessionString, donatedBy);
...
const accountQueries = {
    ...
    updateStatus: db.prepare('UPDATE accounts SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE phone = ?'),
    updateActivity: db.prepare('UPDATE accounts SET last_connected = ?, updated_at = CURRENT_TIMESTAMP WHERE phone = ?')
};
