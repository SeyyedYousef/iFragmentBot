	...
	launchPromise = (async () => {
		...
		try {
			...
			const timeoutPromise = new Promise((_, reject) =>
				setTimeout(
					() => reject(new Error("Browser launch timed out after 30s")),
					30000,
				),
			);
	...
	try {
		...
		try {
			...
			const templatePath = join(
				__dirname,
				"../Templates/flex-card.template.html",
			);
			const _marketTemplatePath = join(
				__dirname,
				"../Templates/market-card.template.html",
			);
...
function replacePlaceholders(html, data) {
	...
	const replacements = {
		...
		"{{STATUS_TEXT}}":
			data.statusText?.replace(/[💰🔨✅❌✨🔒❓]/gu, "").trim() || "Unknown",
		...
		"{{LAST_SALE_TON}}": data.lastSalePrice
			? formatNumber(data.lastSalePrice)
			: "—",
...
function generateChartBars(priceHistory, lastSalePrice) {
	...
	if (priceHistory && Array.isArray(priceHistory) && priceHistory.length > 0) {
		...
		priceHistory.slice(-numBars).forEach((p, i, arr) => {
			...
			bars.push(
				`<div class="chart-bar${isLast ? " highlight" : ""}" style="height: ${heightPercent}%"></div>`,
			);
		});
	...
	const _basePrice = lastSalePrice || 1000;
	const _prices = [];
...
export async function generateNewsCard(data) {
	try {
		...
		try {
			...
			const templatePath = join(
				__dirname,
				"../Templates/news-card.template.html",
			);
			if (!existsSync(templatePath))
...
export async function generateNewsCard2(data) {
	try {
		...
		try {
			...
			const templatePath = join(
				__dirname,
				"../Templates/news-card-v2.template.html",
			);
			if (!existsSync(templatePath))
				throw new Error(`Template not found: ${templatePath}`);
...
export async function generateWalletCard(data) {
	try {
		...
		try {
			...
			const templatePath = join(
				__dirname,
				"../Templates/wallet-card.template.html",
			);
			if (!existsSync(templatePath)) {
				console.warn(
					"⚠️ Wallet card template not found, skipping card generation",
				);
				return null;
			}
			...
			const replacements = {
				"{{ADDRESS}}": data.address
					? `${data.address.substring(0, 8)}...${data.address.slice(-6)}`
					: "Unknown",
