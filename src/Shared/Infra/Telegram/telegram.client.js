...
import {
	...
	loadTelegramSession,
	saveTelegramSession,
} from "../Database/mongo.repository.js"; // Keeping for backward compat/backup
...
// Configuration
const API_ID = parseInt(process.env.TELEGRAM_API_ID, 10) || 0;
 ...
 */
async function _saveSession(session, userInfo = null) {
...
async function submitCode(code) {
	...
	try {
		const _result = await client.invoke(
			new Api.auth.SignIn({
		...
		await accountManager.addSession(
			me.phone || loginState.phoneNumber,
			sessionString,
			{
				id: me.id.toString(),
...
async function getUserGiftsWithValue(userId) {
	...
	try {
		return await accountManager.executeWithSmartRetry(async (client) => {
			...
			console.log(
				`✅ Found total ${allGifts.length} gifts across ${loopCount} pages, total value: ${totalValue} Stars`,
			);
			return {
				success: true,
				totalValue,
		...
		if (error.message.includes("No available clients")) {
			console.warn(
				"⚠️ Gift Scan Skipped: No Telegram accounts available. Add accounts via /panel to enable gift scanning.",
			);
		...
		if (
			...
			console.log(
				"⚠️ payments.GetUserStarGifts not available, trying profile scrape...",
			);
...
async function getUserGiftsFromProfile(userId) {
	try {
		...
		const _fullUser = await useClient.invoke(
...
setTimeout(() => {
	initClient().catch((err) =>
		console.error("Failed to auto-init Telegram client:", err),
	);
}, 3000);
