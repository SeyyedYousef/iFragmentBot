...
export async function scrapeProxies() {
    ...
    try {
        ...
        console.log(`☁️ Fetched ${uniqueProxies.length} unique proxies. Validating...`);
        ...
        for (let i = 0; i < uniqueProxies.length; i += BATCH_SIZE) {
            ...
            if (processed % 500 === 0) {
                console.log(`☁️ Validated ${processed}/${uniqueProxies.length} - Found ${validProxies.length} working`);
            }
        }
        ...
        for (const p of validProxies) {
            ...
            const exists = proxyManager.getAllProxies().some(
