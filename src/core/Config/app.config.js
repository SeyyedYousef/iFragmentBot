...
import { Lexicon, VOWELS, CONSONANTS, BRANDABLE_PATTERNS } from './lexicon.config.js';
...
class LibraryKeeper {
    ...
    loadArchives() {
        ...
        try {
            const fourLettersPath2 = path.join(ROOT_DIR, 'data', '4 Letters ( 2 ).csv');
        ...
        console.log(`📚 Library: ${this.anchors.size} anchors loaded from ${totalLoaded} records (Vol: ${this.totalVolume.toLocaleString()} TON)`);
    }
    ...
    findSimilarSales(username) {
        ...
        return {
            ...
            examples: topMatches.slice(0, 5).map(m => `"${m.name}" (${m.price.toLocaleString()} TON)`)
        };
    }
}
...
const REGIONAL_MARKETS = {
    // 🇷🇺 Russia
    'moscow': 100000, 'russia': 100000, 'rossiya': 75000, 'sber': 100000, 'gazprom': 750000,
    'rosneft': 250000, 'lukoil': 200000, 'kremlin': 150000, 'piter': 50000, 'siberia': 50000,

    // 🇦🇪 Dubai/Gulf
    'dubai': 250000, 'uae': 250000, 'emirate': 150000, 'saudi': 200000, 'riyadh': 100000,
    'falcon': 125000, 'sheikh': 250000, 'khalifa': 150000, 'emaar': 100000, 'burj': 150000,
    'qatar': 150000, 'doha': 100000, 'bahrain': 75000, 'kuwait': 100000,
    ...
    // 🌍 Global Elite
    'london': 200000, 'paris': 150000, 'tokyo': 175000, 'singapore': 150000, 'hongkong': 150000,
    'berlin': 100000, 'zurich': 100000, 'geneva': 100000, 'monaco': 200000, 'milan': 100000
};
...
export class TheOracle {
     ...
     * 9. FINAL MERGE
     */
    static async consult(username, tonPrice = CONFIG.LIVE_TON_PRICE, externalContext = {}) {
        ...
        try {
            ...
            const mContext = {
                ...
                lastSalePrice: externalContext.lastSale || (anchor ? anchor.price : null),
            ...
            if (aiRes && aiRes.valuation && aiRes.valuation.ton > 0) {
                ...
                return {
                    ...
                    rarity: {
                        tier: this.getTier(aiRes.valuation.ton),
                        stars: this.getStars(aiRes.valuation.ton),
                    ...
                    aura: {
                        ...
                        color: this.getAuraColor(aiRes.valuation.ton),
        ...
        if (anchor) {
            baseValue = this.adjustAnchorValue(anchor);
            confidence = 95;
            ...
            if (similarSales && similarSales.count >= 3) {
                ...
                factors.push(`⚖️ Sim. Market: ~${similarSales.median.toLocaleString()} TON`);
        ...
        // Length Premium
        // Length Premium
        const hasMarketData = factors.some(f => f.includes('Sim. Market') || f.includes('Sold Before'));
    ...
    static calculateScarcityBase(length) {
        ...
        return Math.max(1, CONFIG.SCARCITY_MULTIPLIER / Math.pow(length, CONFIG.SCARCITY_EXPONENT));
    }
...
export async function batchEstimate(usernames, tonPrice = CONFIG.LIVE_TON_PRICE) {
    const results = await Promise.all(usernames.map(async u => ({ username: u, ...(await TheOracle.consult(u, tonPrice)) })));
    return results;
}
...
export async function estimateValue(username, lastSale = null, tonPrice = CONFIG.LIVE_TON_PRICE, status = 'Unknown') {
...
export async function calculateRarity(username, existingResult = null) {
    ...
    if (existingResult && existingResult.rarity) {
