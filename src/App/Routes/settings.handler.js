		...
		await ctx.editMessageText(msg, {
			...
			reply_markup: {
				inline_keyboard: [
					...
					[
						{
							text: "🛡️ وضعیت سلامت اکانت‌ها",
							callback_data: "settings_health_guard",
						},
					],
...
برای فعال/غیرفعال کردن روی پروکسی کلیک کنید:
	...
	bot.action(/^toggle_proxy:(\d+)$/, async (ctx) => {
		...
		const proxyId = parseInt(ctx.match[1], 10);
				...
				reply_markup: {
					inline_keyboard: [
						[
							{
								text: "✅ بله، حذف شود",
								callback_data: "confirm_delete_proxies",
							},
        ...
        `.trim(),
			{
				...
				reply_markup: {
					inline_keyboard: [
						...
						[
							{
								text: "⏱️ تنظیم زمان (15 دقیقه)",
								callback_data: "ghost_mode_set:15",
							},
							{
								text: "⏱️ تنظیم زمان (60 دقیقه)",
								callback_data: "ghost_mode_set:60",
							},
						],
						[{ text: "🔙 بازگشت", callback_data: "settings_proxy_cloud" }],
					],
        ...
        `.trim(),
			{
				...
				reply_markup: {
					inline_keyboard: [
						[
							{
								text: newStatus.isEnabled ? "🔴 غیرفعال کردن" : "🟢 فعال کردن",
								callback_data: "ghost_mode_toggle",
							},
						],
						[
...
فاصله زمانی: \`${status.interval} دقیقه\`
	...
	bot.action("settings_test_proxies", async (ctx) => {
		...
		const _testMsg = await ctx.editMessageText(msg, { parse_mode: "Markdown" });
        ...
        `.trim(),
			{
				...
				reply_markup: {
					inline_keyboard: [
						[
							{
								text: separate ? "✅ فعال" : "فعال",
								callback_data: "toggle_separate:1",
							},
							{
								text: !separate ? "✅ غیرفعال" : "غیرفعال",
								callback_data: "toggle_separate:0",
							},
						],
						[{ text: "🔙 بازگشت", callback_data: "panel_settings" }],
					],
