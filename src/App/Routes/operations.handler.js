        ...
        `.trim(),
			{
				...
				reply_markup: {
					inline_keyboard: [
						...
						[
							{
								text: "📥 استخراج از اکانت‌ها",
								callback_data: "ops_extract_profiles",
							},
							{
								text: "📤 اعمال به اکانت‌ها",
								callback_data: "ops_apply_profiles",
							},
						],
...
یوزرنیم کاربرانی که کامنت گذاشته‌اند استخراج خواهد شد.
	...
	bot.action("ops_group_to_group", async (ctx) => {
		...
		await ctx.editMessageText(
			"👥 *گروه به گروه*\n\nاین قابلیت بزودی فعال می‌شود.",
			{
				reply_markup: {
					inline_keyboard: [
						[Markup.button.callback("🔙 بازگشت", "panel_operations")],
					],
				},
			},
		);
	...
	bot.action("ops_auto_exit", async (ctx) => {
		...
		await ctx.answerCbQuery(
			currentState ? "🚪 خروج خودکار غیرفعال شد" : "✅ خروج خودکار فعال شد",
		);
		// Toggle logic here
	});
...
async function handleRangeExtraction(ctx, text) {
	...
	const progressMsg = await ctx.reply(
		`⏳ شروع استخراج از مجموعه *${slug}* (از ${start} تا ${end})...\n\n_این عملیات در پس‌زمینه انجام می‌شود._`,
		{ parse_mode: "Markdown" },
	);
	...
	g2g
		...
		.then((results) => {
			ctx.reply(
				`✅ *استخراج رنج تکمیل شد*\n\n🎁 کالکشن: \`${slug}\`\n📊 کل بررسی شده: \`${results.total}\`\n✅ یافت شده: \`${results.success}\``,
				{ parse_mode: "Markdown" },
			);
		})
		.catch((err) => {
...
async function handleGiftLinks(ctx, text) {
	...
	const progressMsg = await ctx.reply(
		`⏳ در حال استخراج از ${links.length} لینک...`,
	);
...
async function handleInviteLinks(ctx, text) {
	...
	const progressMsg = await ctx.reply(
		`⏳ در حال جوین به ${links.length} لینک...`,
	);
	...
	for (const link of links) {
		for (const account of accounts.slice(0, 5)) {
			try {
				...
				if (client) {
					await client.invoke(
						new (await import("telegram")).Api.messages.ImportChatInvite({
...
async function handleCommentPostLink(ctx, text) {
	...
	try {
		const result = await g2g.extractMembersFromChat(`@${channel}`, {
			limit: 100,
			fromComments: true,
			messageId,
		});
