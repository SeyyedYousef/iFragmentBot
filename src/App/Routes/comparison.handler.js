...
export async function handleComparison(ctx, username1, username2) {
	...
	try {
		...
		const [data1, data2, insight1, insight2] = await Promise.all([
			scrapeFragment(username1).catch((_e) => ({
				statusText: "Error",
				status: "unknown",
			})),
			scrapeFragment(username2).catch((_e) => ({
				statusText: "Error",
				status: "unknown",
			})),
			generateShortInsight(username1).catch((_e) => "No insight"),
			generateShortInsight(username2).catch((_e) => "No insight"),
		]);
		...
		try {
			...
			if (cardError)
				console.error("Story Card gen error (bg):", cardError.message);
			imageBuffer = null;
		}
			...
			reply_markup: {
				inline_keyboard: [
					...
					[
						{
							text: "📸 Share Story",
							callback_data: `story_username:${username1}`,
						},
						{
							text: `📊 @${username2}`,
							callback_data: `view_username:${username2}`,
						},
					],
