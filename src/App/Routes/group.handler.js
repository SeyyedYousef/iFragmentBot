...
import {
	getTonPrice,
	scrapeFragment,
} from "../../Modules/Market/Infrastructure/fragment.repository.js";
 ...
 */
async function _withRetryOnce(fn, label = "Operation") {
	try {
		return await fn();
	} catch (firstError) {
		console.warn(
			`⚠️ ${label} failed, retrying in ${RETRY_DELAY_MS}ms: ${firstError.message}`,
		);
 ...
 */
export function buildUsernameCardData(
	username,
	fragmentData,
	rarity,
	estValue,
...
export async function handleGroupCommand(
	...
	console.log(
		`🛠️ [GroupHandler] Processing: "${input}" (User: ${userId}, Chat: ${ctx.chat.id})`,
	);
	...
	try {
		...
		if (command === "!gifts" || command === "!gift") {
			if (args.length === 0)
				return ctx.reply("⚠️ Usage: `!Gifts <link>`", {
					parse_mode: "Markdown",
				});
			...
			try {
				...
				if (estimatedWait > 5) {
					statusMsg = await ctx.reply(
						formatQueueMessage(position + 1, estimatedWait, false),
						{
							parse_mode: "Markdown",
							reply_to_message_id: ctx.message.message_id,
						},
					...
					statusMsg = await ctx.reply("🔮 Analyzing gift...", {
						reply_to_message_id: ctx.message.message_id,
					});
				}
		...
		// !Number <link|+888XXXXXXXXXX>
			...
			if (args.length === 0)
				return ctx.reply(
					"⚠️ Usage: `!number https://fragment.com/number/8881234567890` or `!number +8881234567890`",
					{ parse_mode: "Markdown" },
				);
			...
			if (!parsed.isValid) {
				return ctx.reply(
					"⚠️ Invalid number format. Use Fragment link or +888XXXXXXXXXX",
					{ reply_to_message_id: ctx.message.message_id },
				);
			}
			...
			try {
				...
				if (estimatedWait > 5) {
					...
					statusMsg = await ctx.reply("📱 Analyzing number...", {
						reply_to_message_id: ctx.message.message_id,
					});
				}
		...
		// !Username <username>
			...
			if (args.length === 0)
				return ctx.reply("⚠️ Usage: `!Username @name`", {
					parse_mode: "Markdown",
				});

			const username = args[0].replace("@", "").toLowerCase();
			if (!isValidUser(username))
				return ctx.reply("⚠️ Invalid username format.", {
					reply_to_message_id: ctx.message.message_id,
				});
			...
			try {
				...
				if (estimatedWait > 5) {
					...
					statusMsg = await ctx.reply(`🔍 Analyzing @${username}...`, {
						reply_to_message_id: ctx.message.message_id,
					});
				}
		...
		// !Compare <user1> <user2>
			...
			if (args.length < 2)
				return ctx.reply("⚠️ Usage: `!Compare @user1 @user2`", {
					parse_mode: "Markdown",
				});
			...
			try {
				...
				if (estimatedWait > 5) {
					...
					await ctx.reply(`⚔️ Comparing @${u1} vs @${u2}...`, {
						reply_to_message_id: ctx.message.message_id,
					});
				}
			} catch (error) {
		...
		// !Wallet <address|username>
			...
			if (args.length === 0)
				return ctx.reply("⚠️ Usage: `!Wallet <address|@username>`", {
					parse_mode: "Markdown",
				});
			...
			if (
				!target.startsWith("UQ") &&
				!target.startsWith("EQ") &&
				target.length < 40
			) {
				...
				if (isValidUser(username)) {
					...
					await ctx.reply(`🔍 Finding owner of @${username}...`, {
						reply_to_message_id: ctx.message.message_id,
					});
					...
					try {
						...
						return ctx.reply(`❌ Error resolving username: ${e.message}`, {
							reply_to_message_id: ctx.message.message_id,
						});
					}
				} else {
					return ctx.reply("⚠️ Invalid wallet address or username.", {
						reply_to_message_id: ctx.message.message_id,
					});
				}
			}
		...
		// !me - Premium Profile Card
			...
			const { getMeCache, saveMeCache } = await import(
			...
			if (
				cachedData &&
				Date.now() - new Date(cachedData.updatedAt).getTime() < SEVEN_DAYS
			) {
				const daysRemaining = Math.ceil(
					(SEVEN_DAYS -
						(Date.now() - new Date(cachedData.updatedAt).getTime())) /
						(24 * 60 * 60 * 1000),
				);
			...
			const statusMsg = await ctx.reply(
				"🎨 Generating your Premium Profile...",
				{ reply_to_message_id: ctx.message.message_id },
			);

			try {
				const telegramClientService = await getTelegramClient();
				const [giftsData, _tonPrice] = await Promise.all([
				...
				if (!giftsData.success) {
					throw new Error(
						"Could not fetch gift data. Make sure your profile is public.",
					);
				}
				...
				const userProfile = await ctx.telegram.getUserProfilePhotos(
					userId,
					0,
					1,
				);
				...
				if (giftsData.gifts && giftsData.gifts.length > 0) {
					crownJewel = giftsData.gifts.reduce((prev, current) =>
...
`;
				...
				const { generateProfileCard } = await import(
					"../../Modules/User/Application/me-card.service.js"
				);
...
export async function handlePortfolioByWallet(ctx, walletAddress) {
	try {
		...
		await ctx.reply(
			"❌ Could not fetch portfolio. Please check the wallet address and try again.",
		);
	}
}
...
export async function handlePortfolioByUsername(ctx, username) {
	...
	try {
		...
		if (!ownerWallet) {
			console.log(
				`⚠️ TonAPI lookup failed for @${username}, trying Fragment scraping...`,
			);
