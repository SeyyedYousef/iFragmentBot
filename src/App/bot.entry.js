...
} from "../App/Routes/admin.handler.js";
import { handleComparison } from "../App/Routes/comparison.handler.js";
import { handleGroupCommand } from "../App/Routes/group.handler.js";
import {
	handleMenuTextMessage,
	registerMenuHandlers,
} from "../App/Routes/menu.handler.js";
...
} from "../App/Routes/operations.handler.js";
import {
	handleSettingsTextMessage,
	registerSettingsHandlers,
} from "../App/Routes/settings.handler.js";
import {
	handleFakePanelTextMessage,
	registerFakePanelHandlers,
} from "../Modules/Admin/Presentation/fake-panel.handler.js";
...
} from "../Modules/User/Presentation/account.handler.js";
...
bot.use(async (ctx, next) => {
	...
	if (
		ctx.chat &&
		(ctx.chat.username === "FragmentInvestors" ||
			String(ctx.chat.id) === "-1002220790800")
	) {
		if (ctx.from && !ctx.from.is_bot) {
			// Check if it's a Paid Message (Stars)
			const isPaid = !!ctx.message?.paid_media;

			// Reward 300 FRG for any message, but we can log if it was paid
			addFrgCredits(
				ctx.from.id,
				300,
				isPaid ? "Paid Group Message" : "Group Message",
			);
...
async function getTelegramClient() {
	if (!telegramClientModule) {
		try {
			telegramClientModule = await import(
...
// Initial values
tonPriceCache.set("marketStats", { price: 5.5, change24h: 0, timestamp: 0 }); // Safe default
tonPriceCache.set("price", 5.5);
...
async function initAndLaunch() {
	...
	getTonMarketStats()
		...
		.catch((_e) => console.error("Initial TON fetch failed"));
	...
	jobQueue.registerHandler(JOB_TYPES.USERNAME_REPORT, async (job) => {
		...
		try {
			...
			// Fresh fetch
			const [fragmentData, currentTonPrice, insight, suggestions] =
				await Promise.all([
			...
			const shareText = encodeURIComponent(
				`💎 Check out the detailed analysis for @${username} on iFragment Bot!\n\n💰 Est. Value: ${estValue.ton.toLocaleString()} TON\n⭐ Rarity: ${rarity.tier}`,
			);
				...
				inline_keyboard: [
					fragmentData.ownerWalletFull
						? [
								{
									text: "💼 Portfolio",
									callback_data: `portfolio:${fragmentData.ownerWalletFull}`,
								},
								{ text: "🔗 Fragment", url: fragmentData.url },
							]
	...
	jobQueue.registerHandler(JOB_TYPES.PORTFOLIO, async (job) => {
		...
		const { generateWalletReport } = await import(
	...
	bot.command("clearcache", async (ctx) => {
		...
		await ctx.reply(
			`✅ Memory cache cleared successfully! (${count} items removed)`,
		);
	});

	// ==================== ME COMMAND (New) ====================
	bot.command("me", async (ctx) => {
		const { handleMeCommand } = await import(
			"../Modules/User/Presentation/me.handler.js"
		);
	...
	for (let attempt = 1; attempt <= 5; attempt++) {
		try {
			...
			console.error(
				`❌ [Bot] Launch Error (Attempt ${attempt}/5):`,
				err.message,
			);
			if (attempt === 5) {
				console.error(
					"❌ [Bot] Final Failure. Check your token or network connection.",
				);
				process.exit(1);
			}
...
bot.use(async (ctx, next) => {
	console.log(
		`📡 [Update] Type: ${ctx.updateType} | From: ${ctx.from?.id || "Unknown"}`,
	);
	return next();
});
...
bot.action("check_membership", async (ctx) => {
	...
	if (isMember) {
		...
		await ctx.answerCbQuery(
			`❌ You are not a member yet! Please join ${CONFIG.REQUIRED_CHANNEL} first.`,
			{ show_alert: true },
		);
		try {
...
bot.on("photo", async (ctx) => {
	...
	if (
		userState &&
		userState.action === "frag_news_2_await_photo" &&
		isAdmin(ctx.from.id)
	) {
		try {
			const photo = ctx.message.photo[ctx.message.photo.length - 1];
								...
								reply_markup: {
									inline_keyboard: [
										...
										[
											{
												text: "❌ Cancel",
												callback_data: "panel_cancel_login",
											},
										],
									],
								},
