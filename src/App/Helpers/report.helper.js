 ...
 */
function drawGradientBar(score) {
    const blocks = ['░', '▒', '▓', '█'];
    const total = 7;
...
export function getPerfectFor(username) {
    ...
    const categories = [
        ...
        { keywords: ['crypto', 'bitcoin', 'nft', 'token', 'coin', 'btc', 'eth', 'ton', 'defi', 'wallet', 'web3', 'chain', 'swap', 'stake'], result: '₿ Crypto/Web3' },
        { keywords: ['shop', 'store', 'buy', 'sell', 'market', 'mall', 'deal', 'sale', 'trade', 'price'], result: '🛒 E-commerce' },
        { keywords: ['news', 'media', 'press', 'daily', 'times', 'post', 'report', 'journal'], result: '📰 Media/News' },
        { keywords: ['tech', 'dev', 'code', 'ai', 'bot', 'app', 'cyber', 'data', 'cloud', 'hack', 'smart', 'digital'], result: '💻 Tech/Dev' },
        { keywords: ['music', 'art', 'film', 'movie', 'song', 'beat', 'studio', 'dj', 'record', 'design'], result: '🎨 Creative' },
        { keywords: ['travel', 'tour', 'fly', 'hotel', 'trip', 'vacation', 'explore', 'world'], result: '✈️ Travel' },
        { keywords: ['food', 'eat', 'cook', 'chef', 'pizza', 'burger', 'coffee', 'cafe', 'restaurant'], result: '🍕 Food/Lifestyle' },
        { keywords: ['fashion', 'style', 'wear', 'brand', 'luxury', 'designer', 'beauty'], result: '👗 Fashion' },
        { keywords: ['health', 'fit', 'gym', 'sport', 'yoga', 'workout', 'wellness'], result: '💪 Fitness' },
        { keywords: ['king', 'queen', 'god', 'boss', 'elite', 'alpha', 'legend', 'master', 'prince', 'lord'], result: '👑 Personal Brand' },
        { keywords: ['wolf', 'lion', 'tiger', 'eagle', 'dragon', 'phoenix', 'fire', 'storm', 'shark', 'hawk'], result: '🔥 Bold Brands' },
        { keywords: ['love', 'heart', 'cute', 'sweet', 'angel', 'beauty', 'dream', 'star', 'moon'], result: '💖 Lifestyle/Social' },
        { keywords: ['bank', 'fund', 'invest', 'finance', 'capital', 'forex', 'stock', 'money', 'cash', 'pay'], result: '🏦 Finance' },
        { keywords: ['edu', 'learn', 'study', 'school', 'course', 'teach', 'tutor', 'academy'], result: '📚 Education' }
    ];
...
function calculateLiquidityScore(username, tier, estValue) {
    ...
    const isAllLetters = /^[a-zA-Z]+$/.test(username);
    const isPronouceable = (username.match(/[aeiou]/gi) || []).length / len >= 0.25;
    ...
    // Tier bonuses
    const tierBonuses = {
        'God Tier': 20, 'Mythic': 15, 'Apex': 10, 'Legendary': 8,
        'Grand': 5, 'Rare': 3, 'Uncommon': 0, 'Common': -5,
        'Scrap': -15, 'Worthless': -25
    };
...
export function buildFullCaption(data, cardData, tonPrice, rarity, estValue, suggestions = []) {
    ...
    const lastSale = data.lastSalePrice ? `${formatNum(data.lastSalePrice)} TON` : '—';
    ...
    if (estValue.linguistics) {
        ...
        if (hasSyl) msg += `${hasPron ? ' • ' : ''}🎼 ${escapeMD(String(syl))} syl\n`;
        else if (hasPron) msg += `\n`;

        const recallVal = Number(estValue.linguistics.recall) || 0;
        const typeVal = Number(estValue.linguistics.typability) || 0;
        if (recallVal > 0) {
            const recallLabel = recallVal >= 9 ? 'Exceptional' : recallVal >= 7 ? 'High' : recallVal >= 4 ? 'Medium' : 'Low';
            msg += `🧠 Recall: *${recallVal}/10* (${recallLabel})\n`;
        }
        if (typeVal > 0) {
            const typeLabel = typeVal >= 9 ? 'Effortless' : typeVal >= 7 ? 'Easy' : typeVal >= 4 ? 'Moderate' : 'Clunky';
...
export function generateComparisonReport(item1, item2, tonPrice) {
    ...
    const tierScores = {
        'God Tier': 10, 'Mythic': 9, 'Apex': 8, 'Legendary': 7,
        'Grand': 6, 'Rare': 5, 'Uncommon': 4, 'Common': 3,
        'Scrap': 2, 'Worthless': 1,
        // Legacy backward compat
        'S-Tier': 9, 'A-Tier': 7, 'B-Tier': 5, 'C-Tier': 3, 'D-Tier': 1
    };
    ...
    const valueRatio = v1.ton > 0 && v2.ton > 0 ? (Math.max(v1.ton, v2.ton) / Math.min(v1.ton, v2.ton)).toFixed(1) : '∞';
    ...
    // Value (weight: 3)
    if (v1.ton > v2.ton) score1 += 3; else if (v2.ton > v1.ton) score2 += 3;
    ...
    // Liquidity (weight: 2)
    if (liq1 > liq2) score1 += 2; else if (liq2 > liq1) score2 += 2;
    ...
    const overallLoser = score1 >= score2 ? u2 : u1;

    const winReasons = [];
    if (v1.ton > v2.ton && score1 > score2) winReasons.push('higher valuation');
    else if (v2.ton > v1.ton && score2 > score1) winReasons.push('higher valuation');
    if (r1Score > r2Score && score1 > score2) winReasons.push('superior rarity');
    else if (r2Score > r1Score && score2 > score1) winReasons.push('superior rarity');
    ...
    const winReason = winReasons.length > 0 ? winReasons.join(' & ') : 'overall advantage';
