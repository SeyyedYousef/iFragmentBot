...
async function apiRequest(endpoint, options = {}) {
    ...
    try {
        ...
        console.log(`📥 See.tg Response: ${response.status} ${response.statusText}`);

        if (!response.ok) {
            console.warn(`See.tg API error: ${response.status} ${response.statusText}`);
            return null;
        }
...
async function getGiftInfo(collectionSlug, itemNumber) {
    ...
    const data = await apiRequest(`/gift?slug=${collectionSlug}&num=${itemNumber}`);
    if (!data) return null;
...
async function getFloorChanges(collectionSlug) {
    ...
    const data24h = await apiRequest(`/floors/changes?slug=${collectionSlug}&period=24h`);
    const data7d = await apiRequest(`/floors/changes?slug=${collectionSlug}&period=7d`);
...
async function getGiftHistory(collectionSlug, itemNumber) {
    ...
    const data = await apiRequest(`/history?slug=${collectionSlug}&num=${itemNumber}`);
    if (!data) return null;
