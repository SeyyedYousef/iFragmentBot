 ...
 */
export function recordSale(collection, itemNumber, price, attributes = {}, timestamp = new Date()) {
    ...
    if (attributes.model) {
        ...
        collectionData.attributes[attributes.model].push({ price, timestamp: timestamp.toISOString() });
    }
 ...
 */
export function getAttributeSales(collection, attributeType, attributeValue, days = 30) {
    const sales = getSalesHistory(collection, days);
    return sales.filter(s => s[attributeType]?.toLowerCase() === attributeValue?.toLowerCase());
}
...
export function calculatePricePrediction(prices) {
    ...
    const variance = cleanPrices.reduce((sum, p) => sum + Math.pow(p - mean, 2), 0) / cleanPrices.length;
...
export function getTrendFactor(collection) {
    ...
    switch (trendData.trend) {
        case 'strong_up': return 1.10;
        case 'up': return 1.05;
        case 'strong_down': return 0.90;
        case 'down': return 0.95;
        default: return 1.0;
    }
}
...
export function getCollectionStats(collection) {
    ...
    return {
        ...
        avgPrice90d: Math.round(allPrices.reduce((a, b) => a + b, 0) / allPrices.length),
...
export function getAttributeComboSales(collection, attributes = {}, days = 60) {
    ...
    const exactMatches = sales.filter(s => {
        ...
        const matchBackdrop = !backdrop || s.backdrop?.toLowerCase() === backdrop.toLowerCase();
        const matchSymbol = !symbol || s.symbol?.toLowerCase() === symbol.toLowerCase();
    ...
    const partialMatches = sales.filter(s => {
        ...
        if (backdrop && s.backdrop?.toLowerCase() === backdrop.toLowerCase()) matches++;
    ...
    const weightedAverage = weightedCount > 0 ? weightedSum / weightedCount : null;
    ...
    return {
        ...
        confidence: exactMatches.length >= 3 ? 'high' : exactMatches.length >= 1 ? 'moderate' : 'low'
    };
}
...
export function calculate7DayForecast(collection) {
    ...
    if (sales.length < 5) {
        return { forecast: null, trend: 'unknown', confidence: 0, changePercent: 0 };
    }
    ...
    const changePercent = Math.round(((forecast - currentPrice) / currentPrice) * 100);
 ...
 */
export function getDemandSupplyScore(collection, currentOnSale = 0, totalItems = 1000) {
    ...
    // Acceleration bonus
    if (acceleration > 1.5) score += 15;  // Demand increasing
    else if (acceleration < 0.5) score -= 10;  // Demand falling

    // Supply scarcity bonus
    if (supplyRatio < 0.05) score += 15;  // Very scarce
 ...
 */
export function getAdvancedValuationData(collection, attributes = {}, currentOnSale = 0, totalItems = 1000) {
    const comboSales = getAttributeComboSales(collection, attributes);
    const forecast = calculate7DayForecast(collection);
    const demandSupply = getDemandSupplyScore(collection, currentOnSale, totalItems);
    ...
    return {
        ...
        stats,
        overallConfidence,
        confidenceLevel: overallConfidence >= 80 ? 'ultra_high' :
            overallConfidence >= 60 ? 'very_high' :
                overallConfidence >= 40 ? 'high' :
                    overallConfidence >= 20 ? 'moderate' : 'low'
    };
}
...
console.log('📈 Sales History Service initialized (EWMA + Trend Analysis + Advanced Valuation)');
